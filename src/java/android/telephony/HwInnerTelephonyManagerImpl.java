...
public class HwInnerTelephonyManagerImpl implements HwInnerTelephonyManager {
    ...
    public int getCardType(int slotId) {
        return HwTelephonyManager.getDefault().getCardType(slotId);
    }

    public void updateCrurrentPhone(int lteSlot) {
        HwTelephonyManager.getDefault().updateCrurrentPhone(lteSlot);
    }

    public void setDefaultDataSlotId(int slotId) {
        HwTelephonyManager.getDefault().setDefaultDataSlotId(slotId);
    }

    public int getLteServiceAbility() {
        return HwTelephonyManager.getDefault().getLteServiceAbility();
    }

    public void setLteServiceAbility(int ability) {
        HwTelephonyManager.getDefault().setLteServiceAbility(ability);
    }

    public boolean isFullNetworkSupported() {
        return HwTelephonyManager.getDefault().isFullNetworkSupported();
    }

    public void printCallingAppNameInfo(boolean enable, Context context) {
        HwTelephonyManager.getDefault().printCallingAppNameInfo(enable, context);
    }

    public String getUniqueDeviceId(int scope) {
        return HwTelephonyManager.getDefault().getUniqueDeviceId(scope);
    }

    public String getCallingAppName(Context context) {
        return HwTelephonyManager.getDefault().getCallingAppName(context);
    }
