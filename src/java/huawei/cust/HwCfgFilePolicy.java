...
public class HwCfgFilePolicy {
    ...
    private static final String[] VERSION_MARK = new String[]{"global_cfg_version", "emui_cfg_version", "pc_cfg_version", WifiEnterpriseConfig.CA_CERT_ALIAS_DELIMITER, "carrier_cfg_version"};
    ...
    static {
        ...
        while (i < MEDIA_DIRS.length) {
            if (MEDIA_DIRS[i].endsWith(CFG_SUFFIX) && (MEDIA_DIRS[i].equals(CFG_SUFFIX) ^ true) != false) {
    ...
    public static File getCfgFile(String fileName, int type) throws NoClassDefFoundError {
        Log.e(TAG, "getCfgFile: Attempt to find " + fileName);
        String[] dirs = type == 1 ? MEDIA_DIRS : CFG_DIRS;
        for (int i = dirs.length - 1; i >= 0; i--) {
            Log.e(TAG, "getCfgFile: Attempt to open " + dirs[i] + fileName);
            File file = new File(dirs[i], fileName);
            if (file.exists()) {
                Log.e(TAG, "getCfgFile: Successfully opened " + dirs[i] + fileName);
                return file;
            }
        }
        return null;
