...
*/
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case 1:
                ...
                try {
                    if (this.mPollingCount != 0 || (hasMessages(6) ^ true) == false) {
    ...
    public void checkIfAllCardsReady() {
        if (!HwVSimUtils.isAllowALSwitch()) {
            ...
            if (IS_SINGLE_CARD_TRAY && (HwCardTrayUtil.isCardTrayOut(0) ^ true) != false && this.mSwitchTypes[0] == 0 && this.mSwitchTypes[1] == 0) {
                if (((!HwVSimUtils.isVSimInProcess() ? HwVSimUtils.isVSimCauseCardReload() : 1) ^ true) != false && isCardHotPlugIn()) {
                    disposeLocalCardStatus();
    ...
    private void switchCommrilModeIfNeeded() {
        ...
        if (!(checkRatCombineModeMatched(currentMode, this.mMainSlot, this.mRatCombineMode) || (IS_HISI_CDMA_SUPPORTED ^ true) == false)) {
